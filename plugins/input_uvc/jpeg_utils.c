...
int compress_image_to_jpeg(struct vdIn *vd, unsigned char *buffer, int size, int quality)
{
    ...
    if (vd->formatIn == V4L2_PIX_FMT_YUYV) {
	...
	} else if (vd->formatIn == V4L2_PIX_FMT_YUV422P) {
		unsigned char *src_y = (unsigned char *)yuyv;
		unsigned char *src_u = (unsigned char *)yuyv + vd->width * vd->height;
		unsigned char *src_v = (unsigned char *)yuyv + vd->width * vd->height * 3 / 2;
        ...
        while(cinfo.next_scanline < vd->height) {
            ...
            for(x = 0; x < vd->width; x++) {
				...
				y =  cinfo.next_scanline * vd->width + x;
				u =  y >> 1;
				v = u;
		
                r = src_y[y] + ((5765 * src_v[v])>>12) - 180;  //r
                g = src_y[y] - ((1415 * src_u[u])>>12) - ((2936 * src_v[v])>>12) + 136;//g
                b = src_y[y] + ((7289 * src_u[u])>>12) - 228;  //b
