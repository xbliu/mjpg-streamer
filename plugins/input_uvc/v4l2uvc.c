                 ...
                 int height, int fps, int format, int grabmethod, globals *pglobal, int id, v4l2_std_id vstd)
{
    ...
    currentFormat.type = vd->capture_type;
...
static int init_v4l2(struct vdIn *vd)
{
	...
	if (vd->cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) {
		vd->capture_type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
		fprintf(stderr,"V4L2_BUF_TYPE_VIDEO_CAPTURE\n");
	} else if (vd->cap.capabilities & V4L2_CAP_VIDEO_CAPTURE_MPLANE) {
		vd->capture_type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
    ...
    memset(&in_struct, 0, sizeof(struct v4l2_input));
    in_struct.index = 0;
    in_struct.type = V4L2_INPUT_TYPE_CAMERA;
    if(ioctl(vd->fd,VIDIOC_S_INPUT,&in_struct) < 0){
        fprintf(stderr," VIDIOC_S_INPUT failed! s_input: %d\n",in_struct.index);
        goto fatal;
    }
    ...
    vd->fmt.type = vd->capture_type;
    vd->fmt.fmt.pix.width = vd->width;
    vd->fmt.fmt.pix.height = vd->height;
    vd->fmt.fmt.pix.pixelformat = vd->formatIn;
    vd->fmt.fmt.pix.field = V4L2_FIELD_ANY;
	fprintf(stderr, "set format: %d res: %d%d type:%d field:%d\n", vd->formatIn, vd->width, vd->height,vd->fmt.type,vd->fmt.fmt.pix.field);
	...
	break;
	case V4L2_PIX_FMT_YUV422P:
		fprintf(stderr, "    ... Falling back to YUV422P mode (consider using -fourcc option). Note that this requires much more CPU power\n");
		vd->formatIn = vd->fmt.fmt.pix.pixelformat;
		break;
      default:
	goto fatal;
	...
	if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == vd->capture_type) {
		struct v4l2_format format;
		format.type = vd->capture_type;
		ret = xioctl(vd->fd, VIDIOC_G_FMT, &format);
		if (ret < 0) {
			fprintf(stderr, "Unable to get format!\n");
			goto fatal;
		}
		vd->nplanes = format.fmt.pix_mp.num_planes;
		
		//vd->buf.length = vd->nplanes;
	}
    ...
    for(i = 0; i < NB_BUFFER; i++) {
        ...
        vd->buf.index = i;
        vd->buf.type = vd->capture_type;
        vd->buf.memory = V4L2_MEMORY_MMAP;
		if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == vd->capture_type) {
			vd->buf.length = vd->nplanes;
			vd->buf.m.planes =  (struct v4l2_plane *)calloc(vd->buf.length, sizeof(struct v4l2_plane));
			if (!vd->buf.m.planes) {
				fprintf(stderr, "Unable to alloc planes!\n");
				goto fatal;
			}
			vd->mem[i]  = (capture_mplanes_t *)calloc(vd->nplanes,sizeof(capture_mplanes_t));
		}
        ...
        if(debug) {
			if (V4L2_BUF_TYPE_VIDEO_CAPTURE == vd->capture_type) {
				fprintf(stderr, "length: %u offset: %u\n", vd->buf.length, vd->buf.m.offset);
			} else {
				fprintf(stderr, "length: %u offset: %u\n", vd->buf.m.planes[0].length, vd->buf.m.planes[0].m.mem_offset);
			}
		}
	    ...
	    if (V4L2_BUF_TYPE_VIDEO_CAPTURE == vd->capture_type) {
			...
			for(int j = 0; j < vd->nplanes; j++){
				capture_mplanes_t *ptr = (capture_mplanes_t *)vd->mem[i];
                ptr[j].length = vd->buf.m.planes[j].length;
                ptr[j].start = (int)mmap(NULL , vd->buf.m.planes[j].length, \
													  PROT_READ | PROT_WRITE, MAP_SHARED , vd->fd, vd->buf.m.planes[j].m.mem_offset);
				
				fprintf(stderr,"start:%x len:%d planes len:%d\n",ptr[j].start,ptr[j].length,vd->buf.m.planes[j].length);
				
				if(ptr[j].start == MAP_FAILED) {
					perror("Unable to map buffer");
					goto fatal;
				}
...
fatal:
	...
	if(vd->capture_type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		free(vd->buf.m.planes);
	}
    return -1;
...
#define HEADERFRAME1 0xaf
	...
	//fprintf(stderr,"vd->buf.bytesused:%d\n",vd->buf.bytesused);
    ...
    case V4L2_PIX_FMT_MJPEG:
        ...
        */
        ...
        if (V4L2_BUF_TYPE_VIDEO_CAPTURE == vd->capture_type) {
			...
			capmp = (capture_mplanes_t *)vd->mem[vd->buf.index];
			memcpy(vd->tmpbuffer,capmp[0].start , vd->buf.bytesused);
		}
	...
	case V4L2_PIX_FMT_YUV422P:
    	if (V4L2_BUF_TYPE_VIDEO_CAPTURE == vd->capture_type) {
			ptr = vd->mem[vd->buf.index];
		} else {			
			capmp = (capture_mplanes_t *)vd->mem[vd->buf.index];
			ptr = capmp[0].start;
			if (0 == vd->buf.bytesused) { //compatible with allwinner
				vd->buf.bytesused = capmp[0].length;
			}
			//fprintf(stderr,"length:%x  byteused:%d\n",capmp[0].length,vd->buf.bytesused);
		}
        if(vd->buf.bytesused > vd->framesizeIn)
            memcpy(vd->framebuffer, ptr, (size_t) vd->framesizeIn);
        else
            memcpy(vd->framebuffer, ptr, (size_t) vd->buf.bytesused);
...
int close_v4l2(struct vdIn *vd)
{
	...
	if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == vd->capture_type) {
		for (int i=0; i<NB_BUFFER; i++) {
			for (int j=0; j<vd->nplanes; j++) {
				capture_mplanes_t *ptr = (capture_mplanes_t *)vd->mem[i];
				munmap(ptr[j].start,ptr[j].length);
			}
		}
		free(vd->buf.m.planes );
	}
